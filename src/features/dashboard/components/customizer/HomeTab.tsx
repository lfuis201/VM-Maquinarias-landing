...
import { HomeVisualEditor } from './HomeVisualEditor';
...
export const HomeTab: React.FC<HomeTabProps> = ({ config, onChange, onSubmit, isLoading }) => {
    ...
    return (
      <div className="min-h-[60vh] w-full flex flex-col items-center justify-center gap-6 bg-white/50 backdrop-blur-sm rounded-3xl border border-slate-200 shadow-sm">
        <div className="relative flex items-center justify-center">
          ...
          <div className="w-16 h-16 rounded-full border-4 border-indigo-100 border-t-indigo-600 animate-spin relative z-10"></div>
  ...
  const bullets = Array.isArray(config.heroBullets)
    ? config.heroBullets
    : typeof config.heroBullets === 'string'
    ? config.heroBullets.split('\n').filter(Boolean)
    : [];

  const handleBulletChange = (idx: number, value: string) => {
    const updated = [...bullets];
    updated[idx] = value;
    onChange('heroBullets', updated);
  };

  const handleAddBullet = () => {
    onChange('heroBullets', [...bullets, 'Nuevo beneficio']);
  };

  const handleRemoveBullet = (idx: number) => {
    const updated = bullets.filter((_: any, i: number) => i !== idx);
    onChange('heroBullets', updated);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
        <div className="text-left">
          <h4 className="text-sm font-bold text-slate-900">Método de Edición de Inicio</h4>

...
import { PoliticaVisualEditor } from './PoliticaVisualEditor';
import { PoliticaFormEditor } from './PoliticaFormEditor';

interface PoliticaTabProps {
  config: any;
...
export const PoliticaTab: React.FC<PoliticaTabProps> = ({ config, onChange, onSubmit, isLoading }) => {
    ...
    return (
      <div className="min-h-[60vh] w-full flex flex-col items-center justify-center gap-6 bg-white/50 backdrop-blur-sm rounded-3xl border border-slate-200 shadow-sm">
        ...
        <div className="space-y-1 text-center">
          ...
          <p className="text-xs text-slate-500 font-medium">Preparando el entorno de personalización de la Política de Privacidad...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
        <div className="text-left">
          <h4 className="text-sm font-bold text-slate-900">Método de Edición de Políticas</h4>
          <p className="text-xs text-slate-500">Edita directamente sobre el diseño de la página o mediante formularios tradicionales.</p>
        </div>
